...
def get_all_hot_money_names() -> list[str]:
    ...
    result = pool.query("SELECT DISTINCT hot_money_name FROM hot_money")
...
def get_hot_money_list() -> list[HotMoney]:
    ...
    result = pool.query("SELECT department_name, hot_money_name FROM hot_money")
    hot_money_list = []
    for row in result:
        hot_money_list.append(HotMoney(department_name=row[0], hot_money_name=row[1]))
        ...
        lhb_stock_list.append(
            LhbStockList(
                ...
                concept_list=[
                    ...
                    for concept in json.loads(row[12])
                ],
            )
        )
...
def get_advance_decline_count(start_date: str, end_date: str) -> pd.DataFrame:
    ...
    if end - start > timedelta(days=31):
        return pd.DataFrame(
            columns=["trade_date", "advance_count", "decline_count", "flat_count"]
        )
    ...
    result_df = pd.DataFrame(
        [
            {
                "trade_date": d,
                "advance_count": results[d]["advance"],
                "decline_count": results[d]["decline"],
                "flat_count": results[d]["flat"],
            }
            for d in dates
        ]
    )
    result_df = result_df[
        (result_df["advance_count"] != 0)
        | (result_df["decline_count"] != 0)
        | (result_df["flat_count"] != 0)
    ]
    return result_df
...
def market_analyze(code: str, date: str) -> MarketAnalyze:
    """
    分析当前指数的均线走势与个股涨跌家数，判断当前行情

    :param code: 代码，如"USHI1A0001"
    :param date: 日期，格式为%Y-%m-%d
    """
    ...
    if date != None:
        ...
        if trade_day.is_trading_day(date) == False:
            date = trade_day.get_latest_trading_day()
    else:
        date = trade_day.get_latest_trading_day()

    minus_ten_days = date
    for i in range(0, 10):
        minus_ten_days = trade_day.get_prev_trading_day(minus_ten_days)
    prices = quote.get_market_index_day_price(code, minus_ten_days, date)
    if len(prices) < 10:
        return None

    ma5 = calculateMA([p.close for p in prices], 5)
    ma10 = calculateMA([p.close for p in prices], 10)

    if ma5 > ma10:
        ret.ma_trend = (
            f"当前{prices[-1].name}的5日线位于10日线之上，为震荡区间/上升趋势"
        )
        if prices[-1].close >= ma5:
            ret.price_trend = f"指数位于5日线之上，存在博弈主升可能"
        elif prices[-1].close < ma5 and prices[-1].close >= ma10:
            ret.price_trend = f"指数位于5日线与10日线之间，需注意防守"
        else:
            ret.price_trend = f"指数跌破10日线，仅考虑抱团品种"
    elif ma5 < ma10:
        ret.ma_trend = (
            f"当前{prices[-1].name}的5日线位于10日线之下，为震荡区间/下降趋势"
        )
        ret.price_trend = f"下降区间，仅考虑抱团品种"
    else:
        ret.ma_trend = f"当前{prices[-1].name}的5日线与10日线重合，无法判断"

    ret.code = prices[-1].code
    ret.name = prices[-1].name
    ret.trade_date = prices[-1].trade_date
    ret.close = prices[-1].close
    ret.ma5 = ma5
    ret.ma10 = ma10
    return ret
...
if __name__ == "__main__":
    ...
    print(market_analyze("USHI1A0001", "2025-07-29"))
