...
export default function Home() {
    ...
    const [selectedTradeDate, setSelectedTradeDate] = React.useState<Dayjs>(dayjs("20250702"));

    useEffect(() => {

        async function fetchQualifiedStocks() {
            try {
                const response = await fetch('/api/quantitative/backtrace?trade_date=' + selectedTradeDate.format('YYYYMMDD'));
                if (!response.ok) {
    ...
    return (
        <Container >
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale='zh-cn'>
                <DatePicker label="回溯日期" value={dayjs(selectedTradeDate)} onChange={val=>val? setSelectedTradeDate(val) : setSelectedTradeDate(dayjs("20250702"))} />
                    ...
                    <Box sx={{ marginBottom: 2 }}>
                        {qualifiedStocks.map((stock) => (
                            <Box key={stock.stock_code} sx={{ marginBottom: 1 }}>
                                ...
                                <TradingViewWidget stock_code={stock.stock_code} quantitativeResult={stock} markerDate={selectedTradeDate.format("YYYY-MM-DD")} />
                            </Box>
                        ))}
                    </Box>
                ) : (
